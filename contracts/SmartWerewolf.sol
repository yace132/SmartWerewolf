...
contract SmartWerewolf {
	...
	function SmartWerewolf(address addressPoK) public {
		...
		emit ReportTime(0,-1, uint(GameIs.Awake));
	}
	...
	function quickDepositGame() external payable {
		...
		gameClock.phase = GameIs.DepositsReady;
		emit ReportTime(0, -1, uint(GameIs.DepositsReady));
	}
	...
	{   
		...
		emit ReportTime(1,-1, uint(GameIs.PlayersReady));
	}
	...
	returns(uint[2][] outDeck){
		...
		//support any shuffle TODO: some shuffle
	}
	...
	function werewolfCard() 
	external 
	view 
	returns(uint cardX,uint cardY){
	...
	function helpDecryptRole(uint me, uint keyInverse/*pokerKey*/, uint i, uint[2] cipherCard) external view returns (uint[2] decryptCard){
		if(me == i){
	...
	function recoverRole(uint me, uint keyInverse/*uint pokerKey*/, uint[2] cipherCard) external view returns (uint[2] decryptCard){
		(decryptCard[0],decryptCard[1]) = multiply(keyInverse, cipherCard);
	}

	function getDecryptKey(uint pokerKey) external view returns (uint keyInverse){
		keyInverse = ECCMath_noconflict.invmod(pokerKey, q);
	}
	...
	*/
	...
	function verifyStatesMessage(
		address playerName,
		bytes32 statesMessage,
		uint8 v,
		bytes32 r,
		bytes32 s
		)
	...
	enum GameIs { Sleep, Awake, DepositsReady, PlayersReady, CardsReady, Night, Day}//executeOnChain, PrepareDeposits, InitPlayers, AssignRoles, Night, Day}
	...
	event ReportTime(uint currentRoom, int currentDay, uint currentPhase);
	...
	event RegLivingPlayer(uint indexed livingPlayerNum, address livingPlayerName);

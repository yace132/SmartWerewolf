...
contract AndPoK {
    ...
    coefficient in Zq*/ 
    uint constant q = 0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFEBAAEDCE6AF48A03BBFD25E8CD0364141;
    
    uint[2] G;
    uint[2] H;

    function AndPoK() public{
       G[0] = Gx;
       G[1] = Gy;
       H[0] = Hx;
       H[1] = Hy;
    ...
    //Eason:
    function prove(uint secret, uint message) public view returns(uint[2] A,uint[2] B,uint M, uint[3] T1, uint[3] T2, uint c, uint s){
        uint[3] memory secretG = Secp256k1_noconflict._mul(secret, G);
        //Eason:  _mul: affine -> Jacobi
        ECCMath_noconflict.toZ1(secretG,p);//Eason: transform Jacobi->affine
        (A[0],A[1]) = (secretG[0],secretG[1]);
        uint[3] memory secretH = Secp256k1_noconflict._mul(secret, H);
        ECCMath_noconflict.toZ1(secretH,p);
        (B[0],B[1]) = (secretH[0],secretH[1]);

        uint t = uint(keccak256(A, B, message)) % q;//Prover chooses a random #
        M = message;
        T1 = Secp256k1_noconflict._mul(t, G);
        ECCMath_noconflict.toZ1(T1,p);
        T2 = Secp256k1_noconflict._mul(t, H);
        ECCMath_noconflict.toZ1(T2,p);
        c = uint(keccak256(A, B, message,T1,T2)) % q;//Verifier chooses a random #
        s = addmod(mulmod(secret,c,q),t,q);
    }

    function verify(uint[2] generator,uint[2] A, uint[3] T, uint s, uint c) internal constant returns (bool) {
    ...
    uint[3] memory LHS = Secp256k1_noconflict._mul(s, generator);
    ...
    /*function test1(uint[2] X) public pure returns(uint) {
    return X[0];
    }*/
    
    function verifyProofs(uint[2] memory A, uint[2] memory B, uint message, uint[3] T1, uint[3] T2, uint s) public view returns (bool) { 

    uint c = uint(keccak256(A, B, message,T1,T2)) % q;

    if (verify(G, A, T1, s, c)!=true) return false;
    if (verify(H, B, T2, s, c)!=true) return false;
    return true;
    }
    
    function test(uint secret, uint message) public view returns(bool result, uint[2] A, uint[2] B ,uint M, uint[3] T1, uint[3] T2, uint c, uint s){
        (A,B,M,T1,T2,c,s)=prove(secret,message);
        result=verifyProofs(A,B,M,T1,T2,s);
    }

    function explainTransform(uint coe) public view returns(uint[3] mul, uint[3] afterTransform){
        mul = Secp256k1_noconflict._mul(coe,G);
        afterTransform = Secp256k1_noconflict._mul(coe,G);
        ECCMath_noconflict.toZ1(afterTransform,p);
    }
}

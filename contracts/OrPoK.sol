...
contract OrPoK {
    ...
    function getMultiple(uint k) public view returns(uint[3]){
        emit Fail(k);
        return Secp256k1_noconflict._mul(k,G);
    ...
    //Eason:
    function createProof(uint pokerKey, uint[2] werewolf, uint[2] liveHand, uint victim, uint[3] TReady,uint cReady) public view returns(uint[2] g, uint[2] A, uint message, uint[3] T, uint c, uint s){
        //input: conditions 
        ...
        if(Secp256k1_noconflict.isPubKey(TReady) && cReady!=0){
            T = TReady;
            c = cReady;
        }else{
            uint t = uint(keccak256(G,A,message)) % q;//add index of proof?
    ...
    function publishProofs(uint pokerKey, uint[2] werewolf, uint[2][] liveHands, uint victim, uint real) public{
        ...
        //iv) create proof
        (pf.g, pf.A, pf.message,) = createProof(pokerKey,werewolf,liveHands[real],victim, realT, realc);
        (, pf.T, pf.c, pf.s) = createProof(pokerKey,werewolf,liveHands[real],victim,realT,realc);
    }

    function publishProofs(uint pokerKey, uint victim, uint real) public{
    ...
    /*function verifyProof(uint[2] g,uint[2] A, uint message, uint[3] T, uint c, uint s) public view returns (bool result) {
    ...
    function verifyProofs() public returns (bool result) { 
        ...
        for(uint i=0; i<proofs.length; i++){
            pf = proofs[i];
            if(!verifyProof(pf.g, pf.A, /*pf.message,*/ pf.T, pf.c, pf.s)) {
                emit Fail(i);
                return false;
            }
    ...
    function testProof(uint pokerKey, uint[2] werewolf, uint[2] liveHand, uint victim) public view returns(bool result, uint[2] _G, uint[2] A, uint message, uint[3] T, uint c, uint s){
        ...
        if(pokerKey != 0){
            //uint secret = pokerKey;
            (_G, A, message,T,c,s) = createProof(pokerKey, werewolf, liveHand, victim, [uint(0),0,0] ,0 );
        }else{
            (_G, A, message,T,c,s) = forgeProof(werewolf, liveHand, victim);
        }
        result=verifyProof(_G, A, /*message,*/ T,c,s);
    }
